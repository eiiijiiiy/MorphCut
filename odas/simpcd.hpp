...
public:
    ...
    void preprocess(int oct_depth, float epsilon_diag)
    {
    ...
    void load(const std::vector<Point> &pts, const std::vector<std::vector<size_t>> &triangles, 
            float sample_space = 0.5, int oct_depth = 4, float epsilon_diag = 0.005)
    {
        std::vector<Point> sampled_pts;
        std::cout << "samping points from triangles ... \n";

        for (int i = 0; i < triangles.size(); i++)
        {
            PMP::internal::triangle_grid_sampling<Kernel>(
                pts[triangles[i][0]], pts[triangles[i][1]], pts[triangles[i][2]],
                sample_space, std::back_inserter(sampled_pts));
        }
        // PMP::sample_triangle_mesh(mesh, std::back_inserter(sampled_pts), 
        //         // CGAL::parameters::use_grid_sampling(true).grid_spacing(sample_space));
        //         CGAL::parameters::use_random_uniform_sampling(true).number_of_points_per_area_unit(10));
        // convert to PCD
        cloud->clear();
        for (auto it = sampled_pts.begin(); it != sampled_pts.end(); ++it)
        {
            PointType p (
                CGAL::to_double(it->x()),
                CGAL::to_double(it->y()),
        ...
        preprocess(oct_depth, epsilon_diag);
#if TEST_SAVE_OCT
        std::stringstream ss;
